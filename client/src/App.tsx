...
return ()=>{
  ...
  return (
    <div className="w-[100vw] h-[100vh] bg-white-500">
      <form  onSubmit={handleJoinChatSubmit} 
        className="">

          <InputLabel inputProps={{
            id:"name",
            onChange:joinChatForm.onChange,value:joinChatForm.formValues.name ,placeholder:"name"}}/>
          <button type="submit"> enter chat</button>
      </form>

      <form onSubmit={handleChatSubmit}> 
      <InputLabel textAreaProps={{
               id:"message",
            onChange:chatForm.onChange,value:chatForm.formValues.message ,placeholder:"message"}}/>

              <button type="submit"> send message</button>
      </form>
      <ul>
         {messages.map(message=>(<li key={message.messageID}>{message.content}</li>))}</ul>
    </div>
  );
}
